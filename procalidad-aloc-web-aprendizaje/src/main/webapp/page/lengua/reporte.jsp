...
<title>Reportes Kumitsari</title>
	...
	<div id="Subheader" style="display: none">
		<div class="container">
			<div class="column one">
				<h1 class="title">Reportes Kumitsari</h1>
			</div>
		</div>
	</div>
									...
									<div class="row">
										<div class="col-sm-12">
											<ul id="lst-idiciaciones">
												<li>Descargue su archivo de seguimiento.</li>
							...
							<aside id="content-perfil"  style="font-family: 'Quicksand',sans-serif;">
								<div class="row">
									<div class="col-md-12 icon-desing-nv" style="font-family: 'Quicksand',sans-serif;">
										<div class="row">
											<div class="col-sm-12">
												<h2 id="title-perfil" style="font-family: 'Quicksand',sans-serif;" class="text-center" style="font-weight: 100">Descargar <b style="font-weight: 500">Reporte</b></h2>
											</div>
										</div>
										<div class="row">
											<div class="col-sm-12">
										...
										<div class="row">
				                                  ...
				                                  <div class="controls">
				                                     <select id="cbTablas" onchange="escogerTabla()" class="form-control"style="height:auto !important" data-validation-required-message="Este campo es requerido" required="required" >
				                                          <option value="" selected disabled="disabled" label="Seleccionar"/>
				                                     		<option value="1">Intentos Por SubNivel</option>
				                                     		<option value="2">Resultado de Evaluaciones</option>
				                                     </select>
				                                  </div>
				                                </div>
				                              </div>
				                              ...
				                              <div class="col-md-12">
				                                	<div class="form-group">
				                                		<!-- href="<c:url value='/seguimiento/descargarPDF'/>"  <a href="${pageContext.request.contextPath}/perfil/descargarArchivoDatos/"  id="enlaceDescargar"   class="btn btn-primary enlaceDescargar" target="_blank" download>Generar Archivo</a> -->
				                                		 <button  id="descargarReport" onclick="descargarReportes();" class="btn btn-primary"  >Generar Reporte</button>
				                                	</div>
				                              </div>
	...
	<script type="text/javascript" charset="utf-8">
		...
		$("#cbAccion").on("change", function() {
		  	...
		  	function descargarReportes(){
		  		var lengua = $("#cbLengua").val();
				 //location.href = url+'?codLen='+lengua;

				 if($("#cbTablas").val()==1 && $("#cbLengua").val()>0 ){
				 modificarElementoGenericoReporte('/seguimiento/descargarPDF',lengua);
				 }else if($("#cbTablas").val()==2 && $("#cbLengua").val()>0){
					 listadoNotas();

				}else{
				//alert("seleccione las opciones de descarga");
					}

		  		}
			function modificarElementoGenericoReporte(url, idElemento){
				var urlComplete = document.getElementById('url_base-proyecto').value + url;
				doPost(urlComplete, { p_codlengua : idElemento } );
			}
			 ...
			 {
			     var url =   "/procalidad-aloc-web-aprendizaje/seguimiento/listanotas";
			     var lengu  =  $("#cbLengua").val();
			     $.ajax({
			    	 async: true,
			       type : 'GET',
			       data :{p_codlengua : lengu },
			       url : url,
			     success : function(data)
			     {
			          if(data.length>0){
			        	  modificarElementoGenericoReporte('/seguimiento/descargarReporteNotas',$("#cbLengua").val());
				          }else{
		...
		function listarLenguas(){
			var url = $("#contextPathUrl").val()+ "/seguimiento/listarLenguaUsuario";
			var html = "";
			$.ajax({
				 async: true,
				type : 'POST',
