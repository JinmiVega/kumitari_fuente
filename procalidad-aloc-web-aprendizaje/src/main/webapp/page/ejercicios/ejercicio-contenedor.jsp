...
<!--[if lt IE 9]>
	...
	<style type="text/css">
		...
		#alternativas #lst-rsp-oracion li{
		    max-width: 150px;
		}
		#lst-oracion .rspt{
...
<body class="page-header-color" onload="ocultarGlosario()">
	...
	<section id="lst-lengua"
		...
		<div class="container">
			 ...
			 <input type="hidden" id="codlenguaValEncrypt" value="${lenguaBean.codigoEncrypt}">
				 <input type="hidden"
				id="nomlenguaTreeVal" value="${lenguaBean.nombre}"> <input
				type="hidden" id="pageActualTreeVal"
				...
				<div class="bseccion">
						...
						<ul>
							<li id="lengua-actual">
								...
								href="${pageContext.request.contextPath}/lengua/detalle/${lenguaBean.codigoEncrypt}">
									<c:choose>
										...
										<c:otherwise>
											<img
												src="${pageContext.request.contextPath}/assets/images/lengua/SIMBOLO-AIMARA.png"
							...
							<li>
								...
								href="${pageContext.request.contextPath}/lengua/detalle/${lenguaBean.codigoEncrypt}/nivel/${lenguaEstructuraBean.codigoEncrypt}">
			   					<span class="circle-item name-leccion tooltipx circle_color_0${lenguaEstructuraBean.nivel.codigoRegistro}">
							...
							<li>
								<%-- <a href="${pageContext.request.contextPath}/rutalengua/quechua/basico/b1/u1">
								<img src="${pageContext.request.contextPath}/assets/images/icono-panel/unidad-1.png" alt="" class="img-responsive" width="62">
			   				</a> --%> <a
								href="${pageContext.request.contextPath}/lengua/detalle/${lenguaBean.codigoEncrypt}/nivel/${lenguaEstructuraBean.codigoEncrypt}/unidad/${unidadBean.codigoEncrypt}">
									<%-- <c:if test="${unidadBean.unidad.codigoRegistro==1}">
						   				<span class="circle-item gris-item-circle">
							   					<span class="name-one">U</span>
							   					<span class="name-two">1</span>
						   				</span>
								</c:if>
								<c:if test="${unidadBean.unidad.codigoRegistro==2}">
						   				<span class="circle-item gris-item-circle">
							   					<span class="name-one">U</span>
							   					<span class="name-two">2</span>
						   				</span>
								</c:if>
								<c:if test="${unidadBean.unidad.codigoRegistro==3}">
						   				<span class="circle-item gris-item-circle">
							   					<span class="name-one">U</span>
							   					<span class="name-two">3</span>
						   				</span>
								</c:if>
								<c:if test="${unidadBean.unidad.codigoRegistro==4}">
						   				<span class="circle-item gris-item-circle">
							   					<span class="name-one">U</span>
							   					<span class="name-two">4</span>
						   				</span>
								</c:if> --%> <span class="circle-item gris-item-circle tooltipx">
							...
							<li>
			   				...
			   				</span> --%> <span class="circle-item name-leccion tooltipx"> <span
									class="name-one"> <c:choose>
											<c:when
								...
								</span> <span class="name-two"> <c:choose>
											<c:when
												test="${unidadLeccionBean.leccion.codigoRegistro == '9'}">
														v
												</c:when>
					...
					<div id="box-masct">
						<input type="hidden" id="idValImgNormal"
							value="${beanMascota.imagenNormal}">
						<input type="hidden"
							id="idValImgAlegre" value="${beanMascota.imagenAlegre}">
						<input type="hidden" id="idValImgTriste"
							value="${beanMascota.imagenTriste}">
						<input type="hidden"
						...
						<input type="hidden" id="glosarioLengua"
							value="${glosarioBean.detalle}">
						<input type="hidden" id="glosarioArchivo"
							value="${pageContext.request.contextPath}/glosario/${glosarioBean.lenguaBean.codigo}_${glosarioBean.fileName}">
						<c:forEach var="mensajes" items="${lstMensajes}">
							<input type="hidden"
								id="msj_${mensajes.tipoMensaje.codigoRegistro}"
								value="${mensajes.mensajes}">
						</c:forEach>
						<div id="bg_fondo_mv" data-movil="false">
							<div id="show-msg"></div>
						</div>
						<c:choose>
			...
			<input type="hidden" id="codSubNivel"
					value="${lenguaEstructuraBean.subNivel.codigoRegistro}">
			<input type="hidden" id="ultimaleccionxunidad"
					value="${ultimaleccionxunidad.codigo}">
			<input type="hidden" id="ultimaleccionxlenest"
					value="${ultimaleccionxlenest.codigo}">
				<input type="hidden" id="codusuariomat"
					value="${usuarioMatriculaBean.codigo}"> <input
					type="hidden" id="codAluMoneG" value="${alumnoMonedaGBean.codigo}">
				...
				<input type="hidden" id="codmaterialValEncrypt" value="${material.codigoEncrypt}">
   				...
   				<input
					type="hidden" id="lenguanivel"
					value="${lenguaEstructuraBean.nivel.codigoRegistro}"> <input
					type="hidden" id="codtipomaterialVal"
					value="${material.situacionLeccionMaterial.codigoRegistro}">
			...
			<div id="id-lista-ejercicio-material" style="display: none;">
				<c:forEach var="materialTipoEj" items="${lstMaterialTipoEjBean}">
					<span class="elementos_ejercicio_material"
						id="${materialTipoEj.codigo}"
						data-tipo="${materialTipoEj.tipoEjercicio.codigoRegistro}"
						data-flg="${materialTipoEj.flgPregu}"></span>
				</c:forEach>
			</div>

			<div id="content-ejercicio-actual-ajax">
				<div id="content-app">
					<!-- Aqui el contenido del material -->
				</div>
				<div id="idDivLoadAjax" style="position: relative;">
					<div class="main-ajax">
	...
	<jsp:include
		page="${pageContext.request.contextPath}/../layouts/ganar-premio-modal-view.jsp"></jsp:include>
	...
	<script
		src="${pageContext.request.contextPath}/assets/js/jquery-1.12.1-ui.min.js"></script>
	...
	<script
		src="${pageContext.request.contextPath}/assets/js/page/ejercicios-script.js"
		type="text/javascript"></script>
	<%-- <script
		src="${pageContext.request.contextPath}/assets/js/page/premio-script.js"
		type="text/javascript"></script> --%>
	<script
		src="${pageContext.request.contextPath}/assets/js/actividad/monedasyGemas.js"
		type="text/javascript"></script>
	<script
		src="${pageContext.request.contextPath}/assets/js/actividad/SeguimientoAlumLen.js"
		type="text/javascript"></script>
	<c:if test="${usuarioEstudiante.codPerfilUsuSelec != 4}">
		<script
