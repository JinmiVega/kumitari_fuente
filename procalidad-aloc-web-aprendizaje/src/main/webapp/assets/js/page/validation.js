  function valida(e){
    ...
    function validar_file(fileid,imgid,imgsrc){
...
function abrir_input(idinput)
{
   document.getElementById(idinput).click();
  	...
  	var nomimgUser = $('#imgnomUser').val();
  //console.log('nomimgUser '+ nomimgUser);
    function limpiar_img(imgid,imgsrc,nomimgUser){ 
    	
      if (nomimgUser=='' || nomimgUser==null) {
      $('#'+imgid).attr('src','../assets/images/avatar.png');
      }else {  
      var nomim = $("#idNombreImg1ti").val();
      var nomimvariada = $("#idNombreImg1v").val(); 
       if (nomim!= null & nomim != ''){
       	$('#'+imgid).attr('src',imgsrc+'hbh');
       }else if(nomimvariada!= null & nomimvariada != ''){
    	   $('#imgmas1v').attr('src',imgsrc+'hgddghdgh');
       }
       else{
       	$('#'+imgid).attr('src',imgsrc+nomimgUser);
       }
