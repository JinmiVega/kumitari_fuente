...
}(jQuery, window));;// Sticky Plugin v1.0.3 for jQuery
// =============
...
$(document).ready(function(){
  ...
  if ($('.progress-indicator-stages').length) {
	...
	// Plugin that detects if an element is visible in the viewport
	...
	var MozillaNB = function () {
		...
		*/
		this.init = function () {
			...
			this.initPopovers();
			this.initGaEvents();
			this.populateUserLanguage();
			this.initPetitionSubmit();
			this.initShareFacebook();
			this.initBablicCustom();
			this.initManageCookies();
			this.initBackstrech();
			this.initLightcase();
			this.initStickySocials();
			this.initFormSteps();
			this.initMemeAnimation();
			this.initScrollDepth();
			this.toggleCheckbox();
			
		};

		this.initScrollDepth = function () {
			$.scrollDepth();
		};
		...
		*/
		this.loadCss = function(url) {
			var stylesheets = [
				//'https://mozillafoundation.nationbuilder.com/themes/4/57dc7fa475dd2f5053000001/0/attachments/14740684731475103716/default/theme.scss',
				...
				'https://cdnjs.cloudflare.com/ajax/libs/lightcase/2.3.4/css/lightcase.min.css',
				'//cdn.jsdelivr.net/jquery.slick/1.6.0/slick.css',
				'//cdn.jsdelivr.net/jquery.slick/1.6.0/slick-theme.css',
				'https://mozillafoundation.nationbuilder.com/themes/4/57dc7fa475dd2f5053000001/0/attachments/14740684731475103716/default/tablet-and-desktop.scss'
			];

			$.each(stylesheets, function(i, el) {
		...
		*
		*/
		this.initFormSteps = function () {
			var $formSteps = $('.form-steps');
			
			// When slick initializes...	
			$formSteps.on("init", function () {
				// Loop through each slick slide and append custom navigation label
				$("[data-slick-index]").each(function (i, el) {
					var index = $(this).attr("data-slick-index");
					var label = $(this).attr("data-label");
					$(this).find("[data-slick-next]").attr('data-slick-next', index);
					$(this).find("[data-slick-back]").attr('data-slick-back', index);

					if (i === 0)
						return;
					
					var nav = '<span class="show-for-medium" data-slick-goto="' + index + '">' + label + '</span>';
					$(".text-nav").append(nav);
			...
			{
				slickOptions.swipe = false;
			}

			// Initialize slick
			$formSteps.slick(slickOptions);

			// Add add attribute to fields in each slick slide for parsley.js validation
			var $sections = $(".row.step");
			$sections.each(function(index, section) {
				$(section).find(':input').attr('data-parsley-group', 'block-' + index);
			});

			// Current slick slide index
			var currentIndex = 0;

			$formSteps.on("beforeChange", function(e, slick, currentSlide, nextSlide) {
			...
			// Validate previous form fields after slide change (mainly for mobile after swipe)
			$formSteps.on('afterChange', function(event, slick, currentSlide){
				...
				{
					/*
					$('[data-slick-goto="' + (currentIndex) + '"]').removeClass("complete");
					return;
					*/

					// Validate last slide
					if ( !$('.petition_signature_form').parsley().validate({group: 'block-' + currentIndex}) )
					{
						// If last slide form fields are invalid, go to last slide
						setTimeout(function () {
							//console.log("form invalid");
							$formSteps.slick("slickGoTo", currentIndex);
						}, 200);
					}
					else
					{
						// Last slide is valid, so update classes of navigation elements
						$('[data-slick-goto]').removeClass("active");
						$('[data-slick-goto="' + currentSlide + '"]').addClass("active");
						$('[data-slick-goto="' + (currentIndex) + '"]').addClass("complete");
				...
				//$('[data-slick-goto="' + (currentSlide) + '"]').addClass("clickable");
			});

			// Custom slick navigation button to next form field
			$(document).on("click", "[data-slick-next]", function (e) {
				e.preventDefault();
				var index = $(this).attr("data-slick-next");
				//console.log(index);

				// If fields in current slide are valid go to the next slide
				if ( $('.petition_signature_form').parsley().validate({group: 'block-' + index}) )
				{	
					if (typeof ga !== "undefined")
						ga('send', 'event', { eventCategory: 'Signup', eventAction: 'Form Step', eventLabel: 'Step '+index});
					$formSteps.slick("slickNext");
				}
			});

			// Custom slick navigation button to previous form field
			$(document).on("click", "[data-slick-back]", function (e) {
				e.preventDefault();
				
				var index = $(this).attr("data-slick-back");
				/*
				//console.log(index);
				...
				if ( $('.petition_signature_form').parsley().validate({group: 'block-' + index}) )
				{
					$formSteps.slick("slickPrev");
				}
				*/
				$('[data-slick-goto="' + index + '"]').removeClass("complete");
				$formSteps.slick("slickPrev");
			});

			// Custom slick navigation labels
			$(document).on("click", "[data-slick-goto].clickable", function (e) {
				e.preventDefault();
				return;
				var nextSlide = $(this).attr('data-slick-goto');

				// If fields in current slide are valid go to the slide selected
				if ( $('.petition_signature_form').parsley().validate({group: 'block-' + currentIndex}) )
				{
					$formSteps.slick("slickGoTo", nextSlide);
				}
			});

			// Show form on small screens...
			$("#show-mobile-petition").on("click", function (e) {
				$(this).hide();
				$formSteps.slick("slickNext");
				$(".petition_signature_form").css({
					"opacity": "0",
					"display": "block"
				});

				setTimeout(function () {
					$(".petition_signature_form").fadeTo('300', 1, function() {
		...
		/**
		* Initialize meme position
		*
		*/
		this.initMemeAnimation = function() {
			var self = this;
		...
		/**
		* Position meme CTA graphics and animate them into view on scroll
		*
		*/
		this.positionMemes = function () {
			var memeOneTop = $("#meme1-position").offset().top;
			//var memeTwoTop = $("#meme2-position").offset().top;
			var memeTwoBottom = $('#footer').height();

			$("#meme1").css({
				"top" : (memeOneTop - 20) + "px"
			});

			$("#meme2").css({
				"top" : "initial",
				"bottom" : (memeTwoBottom + 140) + "px"
			});

			var windowBottom = $(window).scrollTop() + $(window).height();

			var memeOnePosTop = $('#meme1').position().top;
			var memeTwoPosTop = $('#footer').offset().top;

			var memeOneOffset = $(window).height() * 0.45;
			var memeTwoOffset = $(window).height() * 0.35;
			if ((windowBottom - memeOnePosTop) > memeOneOffset)
			{
				$('#meme1').addClass("show");
			}

			if ((windowBottom - memeTwoPosTop) > memeTwoOffset)
			{
				$('#meme2').addClass("show");
			}
		};
		...
		/**
		* Initialize  waypoint for social sticky button
		*
		*/
		this.initStickySocials = function () {
			if (!$('ul.socials:first').isOnScreen() && !$("#petition-form-wrap").isOnScreen())
		...
		/**
		* Initialize all instances backstretch for responsive background images
		*
		*/
		this.initBackstrech = function () {
			$("[data-image]").each(function (i, el) {
		...
		/**
		* Initialize all instances of lightcase for modal content
		*
		*/
		this.initLightcase = function () {
			$("span.media a").attr("data-rel", "lightcase:media");
			$('a[data-rel^=lightcase]').lightcase({
				...
				onFinish: {
					contentLoaded: function() {
						$(".lightcase-contentInner, #lightcase-case").addClass("opaque");

						// Trigger Play Event in GA
						var vidId = $(".lightcase-contentInner video").attr('src').split('mp4-format/');
						if (typeof ga !== "undefined")
							ga('send', 'event', { eventCategory: 'Video', eventAction: 'Video Started', eventLabel: vidId[1]});
						//Track Video Time in GA
						var nextPercentDone = 0.25;
						$(".lightcase-contentInner video").ontimeupdate = function() {
							var vidId = $(this).attr('src').split('mp4-format/');
      						if (this.currentTime/this.duration >= nextPercentDone) {
      							if (typeof ga !== "undefined")
        							ga('send', 'event', { eventCategory: 'Video', eventAction: (nextPercentDone * 100) + "% complete", eventLabel: vidId[1]});
        						nextPercentDone = nextPercentDone + 0.25;
      						}
   			 			};

						$(".lightcase-contentInner video")[0].onended = function () {
							// Track Video End in GA
							var vidId = $(this).attr('src').split('mp4-format/');
							if (typeof ga !== "undefined")
								ga('send', 'event', { eventCategory: 'Video', eventAction: 'Video Ended', eventLabel: vidId[1]});
							lightcase.close();
						};
				...
				onClose: {
					contentRemoved: function () {
						$(".lightcase-contentInner, #lightcase-case").removeClass("opaque");
					}
				}
			});
		};
		...
		this.initManageCookies = function () {
			//console.log(Cookies.get());
		};

		/**
		* Toogle checkbox class for custom background
		*
		*/
		this.toggleCheckbox = function () {

			$(".checkbox input[type='checkbox']").each(function (i, el) {
		...
		*/
		this.initShareLinks = function () {
			$(document).on("click", "[data-share]", function(e) {
                ...
                window.open(url, windowName, 'toolbar=0,status=0'
                    + ',width=' + winWidth
                    + ',height=' + winHeight
                    + ',top=' + winTop
                    + ',left=' + winLeft
		...
		*/
		this.initScrollTo = function () {
			$(document).on("click", "[data-scroll-to]", function(e) {
				...
				$("html, body").animate({
					'scrollTop': Number($(href).offset().top) -2
				}, 300 );
			});
		};
		...
		*/
		this.initModals = function () {
			// Open modals with data-open attribute
			$("[data-modal]").each(function (i, el) {
				var open = $(el)[0].hasAttribute("data-open");
				if (open === true)
					$(el).fadeIn();
			});

			// Close modals
			$(document).on("click", "[data-modal] [data-close]", function(e) {
				e.preventDefault();
				$(this).closest("[data-modal]").fadeOut();
			});
		};

		/**
		* Give petition form fixed positioning when out of view
		*
		*/
		this.initStickyForm = function () {

			if (!$('#petition-form-wrap').isOnScreen())
				$("body").addClass("petition-fixed");
			else
				$("body").removeClass("petition-fixed");
			...
			$('#page-content').waypoint({
				handler: function(direction) {
					...
					{
						$("body").addClass("petition-fixed");
						var petitionHeight = $("#petition-form-wrap").height();
						$("#page-content").css("margin-top", petitionHeight + "px");
						$("#petition-form-wrap").hide().addClass("animated").addClass("fadeInUp").show();
					}
					else
					{
						$("body").removeClass("petition-fixed");
						$("#page-content").css("margin-top", "0");
						$("#petition-form-wrap").removeClass("animated").removeClass("fadeInUp");
					}
				}
			});
		};
		...
		*/
		this.addPlaceholders = function () {
			...
			$("[data-placeholder]").each(function (i, el) {
				var value = $(this).data("placeholder");
				$(this).find("input").attr("placeholder", value);
				$(this).find("select").prepend('<option value="">' + value + '</option>');
		...
		*
		*/
		this.initGaEvents = function () {
			window['optimizely'] = window['optimizely'] || [];

            $(".petition_signature_form").on("submit", function() {
                ga('send', 'event', { eventCategory: 'Signup', eventAction: 'Submitted the form', eventLabel: 'Copyright'});
            });

            $(".socials-bar a").on("click", function() {
                var url = $(this).attr('href').split('?');
                ga('send', 'event', { eventCategory: 'Social', eventAction: 'Clicked on Sidebar Share', eventLabel: url[0]});
            });

            $(".share-buttons a").on("click", function() {
                var url = $(this).attr('href').split('?');
                ga('send', 'event', { eventCategory: 'Social', eventAction: 'Clicked on Modal Share', eventLabel: url[0]});
                // sends a tracking call to Optimizely for the given event name. 
                window.optimizely.push(["trackEvent", "shareClick"]);
            });

            $(".donate-button a").on("click", function() {
                ga('send', 'event', { eventCategory: 'Donate Button', eventAction: 'Clicked the Donate Button', eventLabel: 'Copyright'});
                // sends a tracking call to Optimizely for the given event name. 
                window.optimizely.push(["trackEvent", "donateClick"]);
            });
		};

		/**
		* Populate user language field on AJAX request
		*
		*/
		this.populateUserLanguage = function () {
			
			$( document ).ajaxComplete(function( event,request, settings ) {
				var locale = bablic.getLocale();
				//$("#petition_signature_signup_custom_values_user_language_custom").val(locale);
			});
		};

		/**
		* Find flash message to determine next action
		*
		*/
		this.detectFlashMessage = function () {
			// Trigger petition thanks modal when flash message is available
			if ($(".page-type-petition .flash-share").length < 1)
				return;

			$("#petition-thanks-modal").attr("data-open", "true");
			
		};

		/**
		* Handles petition form submissions
		*	
		*/
		this.initPetitionSubmit = function () {
			$(".petition_signature_form").on("submit", function() {
                
                $(this).find('[type=submit]').attr("disabled", "disabled");
                $(this).find('[type=submit]').parent().hide();
            });
		};
		...
		*	
		*/
		this.initShareFacebook = function () {
			$("[data-facebook]").on("click", function (e) {
				e.preventDefault();
				var link = $("#fb-share-link").text();
				var caption = $("#fb-share-caption").text();
				var picture = $("#fb-share-picture").text();
				var name = $("title").text();
				FB.ui({
					method: 'feed',
					name: name,
		...
		/**
		* Run custom code for Bablic
		*
		*/
		this.initBablicCustom = function () {
			var self = this;
			...
			$('.text.sign-field').val(bablic.__('Show your support'));

			$.each(selects, function (i, el) {
				var select = document.querySelector(el);

				self.sortCountryOptions(el);

				bablic.on('locale',function() {
					locale = bablic.getLocale();
					$("#petition_signature_signup_custom_values_user_language_custom").val(locale);
				...
				bablic.on('translated',function() {
					var text = bablic.__('Show your support');
					$('.text.sign-field').val(text);
		...
		/**
		* Alphabetize country select options
		* @param {string}
		*/
		this.sortCountryOptions = function (selector) {
			...
			$options.each(function (i, el) {
				...
				if ( $(el)[0].hasAttribute("data-other") )
				{
					otherOption = '<option value="other" data-other>' + label + '</option>';
				}
				else
				{
					options.push({
			...
			$.each(sortedOptions, function(i, el) {
				$(selector).append('<option value="' + el.value +'">' + el.label + '</option>');
			});
			$(selector).append(otherOption);

			//console.log(JSON.stringify(sortedOptions));
		};
	};

	window.MozillaNB = new MozillaNB();

	// jQuery document ready
	$(function () {
		window.MozillaNB.init();
	});

	// Fade in body when page loads
	$(window).load(function () {
		window.MozillaNB.loadCss();
		setTimeout(function () {
			$("body").fadeTo('100', 1);
		}, 200);
	});
